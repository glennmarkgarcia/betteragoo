...
// (a visible -> hidden -> visible dip). It also counts full-page navigations so a
// first-install service-worker reload cannot remove and recreate the popup while
// escaping the per-document opacity probe. Show-once persistence, centering,
// dismiss, and absence of console errors are verified as well.
...
try {
  for (const vp of VIEWPORTS) {
    ...
    try {
      ...
      // Wait for first-install activation. The worker may claim this page, but the
      // claim must not trigger a second top-level navigation.
      await page.evaluate(() =>
      ...
      ok(
        ...
        `first service-worker install does not reload the page (navigations=${mainFrameNavigations})`
      );
      ok(
        jsErrors.length === 0,
